...
def ProcessDistances(cluster):
    ...
    for date in ListDates(cluster):
        # Get distances from file given by gaia archive
        ...
        cluster_params = Fit(cluster, date, distances)
        sample_params.append([len(distances), cluster_params[0], cluster_params[1]**2, cluster_params[2]])

    PopulationValues(cluster, all_distances, sample_params)
...
def Fit(cluster, date, distances):
    ...
    filename = 'output/' + cluster + '/' + date + '/distance_distribution.png'
...
def PopulationValues(cluster, all_distances, sample_params):
    ...
    y, x, _ = plt.hist(all_distances, bins=100, range=[low_lim, up_lim], color='#3f3f3f')
    x = (x[1:] + x[:-1]) / 2  # for len(x)==len(y)
    plt.clf()
    ...
    plt.plot(x, superpose(x, sample_params), color='#f44242', ls='-', lw=4)
    ...
    filename = 'output/' + cluster + '/distance_distribution.png'
    plt.savefig(filename)
    plt.clf()
