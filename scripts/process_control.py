...
class ProcessControl:
    ...
    def SingleCluster_SingleDate(self, cluster, date):
        if self.app.matchCheck.isChecked():
            self._ProcessMatch(cluster, date)
        if self.app.lowErrorCheck.isChecked():
            self._ProcessLowError(cluster, date, False)
        if self.app.befilterCheck.isChecked():
            self._ProcessBeFilter(cluster, date)
        if self.app.plotCheck.isChecked():
            self._ProcessPlot(cluster, date)
        if self.app.scaleCheck.isChecked():
            self._ProcessScale(cluster, date)
    ...
    def SingleCluster_AllDates(self, cluster):
        ...
        if self.app.befilterCheck.isChecked():
            for date in dates:
                self._ProcessBeFilter(cluster, date, False)

        if self.app.scaleCheck.isChecked():
            for date in dates:
                self._ProcessScale(cluster, date, baseDate)
            self._Rescale(cluster)

        # Analyze newly scaled photometry
        if self.app.lowErrorCheck.isChecked():
            for date in dates:
                self._ProcessLowError(cluster, date, True)

        if self.app.befilterCheck.isChecked():
            for date in dates:
                self._ProcessBeFilter(cluster, date, True)
    ...
    def _ProcessMatch(self, cluster, date):
    ...
    def _ProcessLowError(self, cluster, date, scaled):
        ProcessLowError(cluster, date, self.app, scaled)

    def _ProcessBeFilter(self, cluster, date, scaled):
    ...
    def _ProcessPlot(self, cluster, date):
    ...
    def _ProcessScale(self, cluster, date, baseDate):
    ...
    def _ApplyScale(self, cluster, date):
